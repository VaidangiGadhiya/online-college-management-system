"""
URL configuration for collegeManagement project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/4.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path
from managementApp import views
from django.conf import settings  
from django.conf.urls.static import static  

admin.site.site_header = 'Management System'                    
admin.site.index_title = 'College Management System'                
admin.site.site_title = 'Management System'

urlpatterns = [
    path('admin/', admin.site.urls),
    path('login',views.login),
    path('signin',views.signin),
    path('home',views.home),
    path('about',views.about),
    path('contact',views.contact),
    path('placement',views.placement),
    path('campus',views.campus),
    path('depcou',views.depcou),
    path('AC',views.ac),
    path('contactdata',views.contactdata),
    path('admissiondata',views.admissiondata),
    path('register',views.registerdata),
    path('logincheck',views.check),
    path('newpassword',views.latest),
    path('changepwd',views.changepwd),
    
] + static(settings.MEDIA_URL,document_root=settings.MEDIA_ROOT)  